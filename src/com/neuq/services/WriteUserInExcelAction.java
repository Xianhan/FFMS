...
public class WriteUserInExcelAction extends HttpServlet {
...
//		}
		...
		List<Income> outAllList =(ArrayList<Income>) session.getAttribute("inarr");
		// 创建工作薄
		...
		for (int j = 1; j < outAllList.size() + 1; j++) {
			...
			Label content2 = new Label(1, j, outAllList.get(j-1).getIncomeDate());
			Label content3 = new Label(2, j, outAllList.get(j-1).getIotype().getType());
			Label content4 = new Label(3, j, outAllList.get(j-1).getIncomeMoney() + "");
